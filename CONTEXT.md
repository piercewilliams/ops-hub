...
- **Data pipeline FULLY AUTOMATED (2026-04-19 hardened):** Mon 9am CDT `snowflake-tracker-sync.yml` — `ingest_tracker.py` → `model_tracker.py` → `enrich_tracker.py` (all via RSA key auth). Mon 2pm CDT `weekly_ingest.yml` in data-headlines — `download_tarrow.py` → `snowflake_enrich.py` → `generate_site.py` → `update_snapshots.py`. Five-hour buffer lets Snowflake rebuild before data-headlines pulls. See **[PIPELINE.md](PIPELINE.md)** for full end-to-end reference. Safety gates added 2026-04-19: required-column check + 500-row floor in ingest/model/snowflake_enrich; env-var emptiness validation in both workflows; timeout-minutes on every step; author-alias normalization in model_tracker.py base CTE. `TRACKER_ENRICHED` in `MCC_PRESENTATION.CONTENT_SCALING_AGENT` ready for Chad/Sigma. Revenue (Naviga+GAM) — Ryan Spalding meeting Mon 2026-04-20.
...
- **Ticket status EOD 2026-04-17:** **PGS-82 IN QA** (Jonathan Gonzalvo — Marcelo transitioned 2026-04-17); **PGS-104 IN QA** (Victor Suarez); **PGS-147 IN QA** (Daury Caba — Saner transitioned 2026-04-17); **PGS-17 CODE REVIEW** (Daury); **PGS-67 CODE REVIEW** (Daury); **PGS-135 CODE REVIEW**; **PGS-139 CODE REVIEW**; **PGS-140 IN QA**; **PGS-111 IN QA**; **PGS-148 IN PROGRESS** (SEO Field Rules); **PGS-141 IN PROGRESS** (H1 Rules — National Team); **PGS-150 ON HOLD**; **PGS-189 SELECTED FOR DEV** (Oliver Felix); **PGS-170 DONE** (WordPress httpx 301 fix — Victor, shipped 2026-04-16); PTECH-7730 TO DO (Julia Kim PM). **National team launch: next week** (Kathryn confirmed 2026-04-17 — showstopper bugs). **Dev team pausing new features** to clear code review backlog (~37 PRs; Saner Keles, 2026-04-17). Small bug exception: Friday releases OK for bug-only tickets.
...
| 2 | Dashboard Instrumentation | 4 | In progress | v0.88 live. National team launch next week. PGS-82/104/147/140/111 all In QA. PGS-17/67/135/139 Code Review. Dev backlog push underway. |
| 3 | Headlines Analysis | 4 | In progress | 13 findings, daily grader, weekly auto-ingest Snowflake-driven (Tarrow is optional supplement). Pending: scrub defunct pubs (First for Women, Soaps in Depth, InTouch); verify Week # / McClatchy 4-4-5 calendar alignment. |
...
- [x] ~~**Ping Chad Bruton — Sigma GRANTs (Track B)**~~ — SENT 2026-04-19. Full GRANT block for USAGE + SELECT + FUTURE TABLES on MCC_PRESENTATION.CONTENT_SCALING_AGENT for both DATA_ENGINEER_L and DATA_ENGINEER_M handed off. Waiting on Chad to execute.
...
- Chad Bruton: execute the Sigma GRANT block on CONTENT_SCALING_AGENT (sent 2026-04-19) — unblocks TRACKER_ENRICHED workbook build
