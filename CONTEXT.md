...
- **Sigma "Site Dashboard - Audience" (shared 2026-04-10 by Deedra Lawhead, per Chris):** This is the editor dashboard template — article-level PVs + SubPVs (subscriber PVs) per story, filterable by date/newsroom/site/section, going back to March 2023. Chris says "you'll soon have an upgraded CSA dash like the editor dash but limited to those working on CSA content." **This is the target state for P2.** The CSA version needs these base columns plus: variant type (persona+format), canonical article / cluster ID (needs EGS-127), platform sent to (PTECH-7641), differentiation score (PGS-140), CSA user. SubPVs are the key revenue signal — measure subscriber engagement on syndication platforms. EGS-127 is the critical dependency: without canonical article tracking, you can't connect CSA variant PVs back to cluster performance.
