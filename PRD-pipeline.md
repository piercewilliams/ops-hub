...
- **Modular quality gates.** Each pipeline element—citations, keywords, brand/author voice, internal links, SEO metadata, cluster assignment, persona match, format compliance, and perhaps more—has a discrete, verifiable quality gate surfaced as a green check or red flag. The editor's job is to fix what failed, not to retry the check. When a module flags a problem, the editor addresses it in the content; the system re-checks automatically when the content is saved or submitted. This prevents the gates from becoming checkbox theater: if modules can be re-run on demand without changing anything, a probabilistic module will eventually pass on the same flawed content, and the gate means nothing. *Open design question: should an editor who has just fixed a specific issue be able to trigger a targeted re-check of that module alone, without waiting for full submission? This is a reasonable UX affordance and the answer should be decided during editorial interface design.*
...
The pipeline isn't a single workflow; it's four parallel workflows running simultaneously, differentiated by content type, automation level, editorial involvement, and channel target. All four require the same foundational infrastructure (article format templates, keywords, publish to CMS) to function. Author profiles apply to T2, T3, and T4; T1 automated content won't carry a named author byline and doesn't require voice-trained author profiles.
...
**Accepts structured briefs.** The CSA receives a fully-specified brief: keywords, target persona, article format template, outlet parameters, distribution channel, pipeline tier, and cluster assignment (if applicable). The brief is either auto-generated from the signal layer (T1/T2) or filled in by an assisted editor (T3/T4). Without a complete brief, generation doesn't start.
...
The Operations Layer is the primary deliverable of this document: the control room Chris describes. It's a unified, real-time dashboard covering the entire pipeline's state, health, and performance: operational (what needs attention now) and strategic (are we building the right things and are they working).
...
- Near-term performance at 24h/48h/7d by channel and content type; this feeds directly into Signal Feed brief suggestions
...
The hero metric. Cost Per Asset is what the entire pipeline is optimizing for, and the CPA Tracker makes that optimization legible over time.
...
The accountability view. This is how Pierce's team holds the dev team to the six-month pipeline vision. Strategic features are tracked here, separately from bugs and routine backlog items; bugs don't consume strategic roadmap bandwidth.
