...
| PGS-174 / PGS-111: All Teams: SEO metadata fields not displayed at Step 3 for Content Format variants | **New ticket (2026-04-15)** | CSA eng | PGS-174 is the new All Teams scoped ticket superseding PGS-111. Bug: SEO/meta title, SEO/meta keywords, SEO/meta description not visible at Step 3 when a Content Format is selected. Confirmed missing for Discovery and What to Know on staging; all other Content Formats to be checked during investigation. Root cause: PGS-88 (shipped v0.82.0) didn't extend to Content Format variant types. Blocks QA of PGS-104 (keywords field — one AC depends on these fields being visible at Step 3). Fix must confirm no regression for standard Audience Variants (which render correctly). AC: fields visible for all Content Format variants; consistent with standard Audience Variant behavior per PGS-88. |
