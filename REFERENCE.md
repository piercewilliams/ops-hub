...
|--------|--------|-------|-------|
| PGS-150: Spike — Investigate Plagiarism Detection Options for CSA-Generated Content | **PRODUCT REVIEW (2026-04-10)** | CSA eng | Pierce CC'd. Scope: off-the-shelf APIs (Copyscape, CopyLeaks, Turnitin, iThenticate) + internal tooling. Surface at Audience Variants stage alongside PGS-82 diff score. Distinct from PGS-82 (internal variant-to-variant): this is output vs. third-party web. National team first. Emily Bohnet surfaced in B2B feedback. Eng produces recommendation with tradeoffs. |
...
| PGS-147: Add Intro Length Requirement for National Team (~80–100 Words) | **CODE REVIEW (2026-04-08)** | Daury Caba completed implementation. Array-based org-specific instruction system — extra instructions stored per-team, loaded based on requesting org, making it reusable for other orgs. PR under review. |
...
| PGS-140: Duplicate Content (Differentiation) Analytics | **IN PROGRESS (2026-04-10)** | Track pass/fail rate, failing variant pairs, reanalysis clicks, CSA auto-regeneration events. Marcelo on Amplitude implementation (intelligence only, no UI). Feeds P2 Amplitude adapter once live. @Pierce Williams + @Jim Robinson tagged. |
...
| PGS-98: Investigate missing impact of Additional Context and Editorial Notes on output | **PRODUCT REVIEW (2026-04-10)** | CSA eng | **Substantive bug** — user-provided context in these fields does not influence generated output. Example from ticket: user specified "focus on an Instagram post" in Additional Context; generated output ignored the post entirely. Likely either prompt injection failure or insufficient weighting. Affects the editorial direction workflow broadly — this is how editors inject their intelligence into the pipeline. |
...
| EGS-127: Tagging + Analytics — Track Content Variant Origins | **IN PROGRESS (2026-04-08)** | Marcelo Freitas | Epic with 4 subtasks. Tracks which content variants came from which canonical article — Susannah's #1 analytics priority. Currently one editor (Sarah) manually tracking 700+ stories/month. Saner located it in backlog during standup; shared with Susannah. Marcelo assigned to start. Distinct from PGS-140 (duplicate content analytics) and PGS-80 (user event tracking). |
