...
export const COMPLETED_TASKS = [
  { date: '2026-04-19', task: 'Sigma creator seat resolved (Track A). Remaining gate to stand up CSA workbooks is Track B only — Snowflake GRANT on MCC_PRESENTATION.CONTENT_SCALING_AGENT to DATA_ENGINEER_L + DATA_ENGINEER_M (USAGE + SELECT + FUTURE TABLES). Full GRANT block handed to Chad Bruton 2026-04-19 with diagnostic instruction to check SHOW GRANTS ON SCHEMA first. Pierce can start building Sarah Price\'s workbook against TRACKER_ENRICHED (~1,900 rows, 70 cols) the moment Chad executes.', project: '#1 Platform Access & Training' },
  { date: '2026-04-19', task: 'Pipeline hardened end-to-end with safety gates, freshness checks, and workflow guardrails. Scripts: ingest_tracker.py enforces REQUIRED_SHEET_COLUMNS + MIN_SAFE_ROWS=500 before any TRUNCATE; model_tracker.py aborts if TRACKER_ENRICHED builds <500 rows and wraps connection in try/finally; enrich_tracker.py centralizes BENCHMARK_START_DATE as a constant; snowflake_enrich.py aborts the data-headlines build if TRACKER_ENRICHED returns <500 rows (primary freshness gate catching a broken Monday-morning sync); generate_site.py engagement analysis downgrades cleanly when AT_COL∩Total Views drops below 30 so pearsonr/qcut can\'t crash the build. Workflows: both YAMLs validate secrets are non-empty and decode to non-empty files before Python runs; timeout-minutes added to every step. No manual recovery needed when a stage fails — the prior good state persists.', project: '#7 Vallone Tracker / CMS Automation' },
  { date: '2026-04-19', task: 'Author name canonicalization propagated to the Snowflake base CTE. model_tracker.py tracker CTE now applies CASE TRIM(AUTHOR) WHEN \'Lauren J-G\' THEN \'Lauren JG\' (+ Lauren Jarvis-Gibson and Hanna WIckes) — previously per-author aggregates in TRACKER_ENRICHED (author_article_count, author_hit_rate, author_avg_pvs) fragmented across spellings. Now authoritative canonical names flow from Snowflake to every downstream consumer. _AUTHOR_ALIASES also corrected in enrich_tracker.py Trends-tab per-author table and in data-headlines generate_site.py + generate_grader.py. national-portfolio.js AUTHORS list updated (Lauren JG canonical).', project: '#7 Vallone Tracker / CMS Automation' },
  { date: '2026-04-19', task: 'cluster_avg_sim_desc (Sara\'s tracker col AN) now written to cluster parent row only — never to child rows. PARENT_ONLY_EXTRAS set in enrich_tracker.py lists columns that should only appear on the cluster anchor; child rows (cluster_id=="") receive empty string. The blanking check now uses truthy semantics so header-row/edge-case rows with no cluster_stats entry are also correctly cleared.', project: '#7 Vallone Tracker / CMS Automation' },
  { date: '2026-04-19', task: 'PIPELINE.md written as canonical end-to-end reference (ops-hub root). Covers both chained GitHub Actions workflows (Mon 9am CDT snowflake-tracker-sync.yml + Mon 2pm CDT data-headlines/weekly_ingest.yml), all seven stages with purpose/source/target/safety gates/runtime, credentials + rotation, failure modes and recovery, adding new data sources, portfolio scope (13 domains), what the pipeline does NOT do (Tarrow doesn\'t flow to Sara\'s sheet; no real-time sync; no revenue data yet), known tech debt, and ownership. SNOWFLAKE.md gets new subsections for author canonicalization (4-location sync requirement) and National Content team portfolio scope, plus the missing ARTICLE_DOMAIN column.', project: '#7 Vallone Tracker / CMS Automation' },
  { date: '2026-04-19', task: 'Headlines Analysis site debranded and dated. All T1 references removed site-wide (page titles, h1/hero, tile claims, caveats, tooltips) across generate_site.py + five supporting generators — site is no longer scope-exclusive to that tier. Run-date display now shows month/day/year ("April 19, 2026") instead of month/year only — applied to main page header, footer, caveats, and experiments "Run:" label; archive labels stay monthly since archives group by month. Backwards-compat regex still strips "T1 Headline Analysis ·" prefix from legacy archived pages.', project: '#3 Headlines Analysis' },
  { date: '2026-04-19', task: 'data-headlines Tarrow pipeline fully replaced with Snowflake automation. snowflake_enrich.py expanded to 31-column pull (yahoo_pvs, amp_pvs, newsletter_pvs, article_access_type, publication_date, vertical, cluster stats, article_domain). generate_site.py Snowflake fallback built: _build_sf_an_2026() generates synthetic AN DataFrame from TRACKER_ENRICHED when XLSX absent. weekly_ingest.yml rescheduled Monday 2pm CDT, fully Snowflake-driven. Six KeyError/empty-array crashes hardened. Full pipeline tested both with and without 2026 XLSX — clean on both paths.', project: '#3 Headlines Analysis' },
  { date: '2026-04-19', task: 'Snowflake TRACKER_ENRICHED pipeline completed and verified end-to-end: ingest (2,108 rows) → model (1,943 rows, 70 cols) → enrich (1,907 matched) → snapshot (TRACKER_WEEKLY). article_domain propagated through full stack: model_tracker.py base CTE + final SELECT, snapshot_tracker.py INSERT/SELECT, create_tracker_weekly.py DDL, and TRACKER_WEEKLY ALTER TABLE. SNOWFLAKE.md written as comprehensive 17-section canonical reference.', project: '#7 Vallone Tracker / CMS Automation' },
  ...
  { date: '2026-04-18', task: 'tarrow_backfill.py built and live. Reads Tarrow Apple News (Publisher Article ID) + SmartNews (url) XLSX tabs, matches against Sara\'s tracker Published URL/Link, and fills empty Syndication platform cells (never overwrites). First run: 13 rows filled. Exports Tracker Template.xlsx for generate_site.py. Wired into weekly_ingest.yml: runs Tuesday after download_tarrow.py, before snowflake_enrich.py. Writes data/tarrow_backfill_report.json per run. Pipeline sequence is now: download → backfill → snowflake_enrich → generate_site → update_snapshots.', project: '#3 Headlines Analysis' },
  ...
  { date: '2026-04-01', task: 'T1 analysis complete: 8 findings live (incl. ANP bottom-performer analysis), 4-tile playbook, weekly ANP cadence from Tarrow established. Sarah Price request fulfilled.', project: '#3 Headlines Analysis' },
  ...
  { date: '2026-03-31', task: 'MSN full-year 2025 + SmartNews 2026 category columns fully wired into analysis pipeline. Both Tarrow data items now complete.', project: '#3 Headlines Analysis' },
];
...
export const PROJECTS = {
  ...
  'p3-headlines': {
    ...
    name: 'Headlines Analysis (Price)',
    owner: 'Pierce · Sarah Price',
    compassGoal: 'G3 — Quality Framework & Testing Protocols',
    description: 'Data analysis and tooling to identify what makes high-performing headlines across the National Content team portfolio. Deliverables: findings playbook, per-author performance profiles, and a daily automated Headline Grader (15 criteria, weekly ingest). Run in partnership with Sarah Price.',
    blockers: [
