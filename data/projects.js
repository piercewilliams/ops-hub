...
export const PROJECTS = {
  ...
  'p1-access': {
    ...
    systems: [
      { name: 'Snowflake / Sigma', status: 'pending', note: 'Sigma access confirmed 2026-04-09 — need to verify all required workbooks/views are accessible (growth_and_strategy_role scope). Schedule Chad Bruton walkthrough to confirm. API credentials (OAuth2 for dashboard adapter) still needed from Chad/IT.' },
    ...
    blockers: [
      'Sigma: confirm full access to required workbooks/views within growth_and_strategy_role — schedule Chad Bruton walkthrough',
    ],
    nextActions: [
      'Schedule Chad Bruton walkthrough of growth_and_strategy_role Snowflake/Sigma data — confirm all needed views accessible, then get OAuth2 credentials for dashboard adapter',
    ],
    dependsOn: [],
  },
  ...
  'p2-dashboard': {
    ...
    owner: 'Pierce',
    description: 'Activate live data ingestion layer in csa-dashboard. Manual metrics bridge live via manual-metrics.js + seedMetrics(). PTECH-7641 DONE (2026-04-08, Platform Technology team): foundational CSA data layer properties now shipping — csa_canonical_article_id, csa_variant_type, csa_variant_id pushed to window.dataLayer for L&E/WP/CUE. Distinct from PGS-80 (CSA eng user event tracking). WordPress send-to WP LIVE (2026-04-09): Us Weekly, Woman\'s World, Soap Opera Digest, Bargain Hunter — content creators can push CSA drafts to WP as drafts; CUE send-to still in progress. PGS-140 (Duplicate Content analytics) → IN PROGRESS as of 2026-04-08 (Marcelo; Susannah retrieved from backlog, made high priority). Amplitude/Claude integration meeting with Amanda Hamilton pending — Sarah Price adding Pierce. p-tagging bug (CUE/WP format mismatch) still needs CSA eng fix before Amplitude data is reliable. Sigma and Marfeel still blocked on credentials.',
    adapters: [
    ...
    nextActions: [
      ...
      'Attend Amplitude/Amanda Hamilton meeting — Sarah Price adding Pierce; she manages Amplitude + has open question on Amplitude/Claude integration',
      'Monitor PGS-140 (duplicate content analytics) — IN PROGRESS as of 2026-04-08 (Marcelo); confirm Amplitude event names with dev team when ticket nears build',
    ],
    dependsOn: ['p1-access'],
  },
  ...
  'p3-headlines': {
    ...
    description: 'Phase 2 active — 13 findings live, 5-tile playbook, author playbooks, experiments page, governor system, Headline Grader all shipped. March + April Tarrow data ingested; 28-column SmartNews article-level data integrated (per-channel views for 11 channels; 86.3% of T1 views from Top feed). Full ANP data pipeline (420K rows, weekly drops). Findings (original 8): Featured Targeting, Push Notifications, Platform Topic Inversion, Views vs. Engagement, Formula Trends, Featuring Reaches Non-Subscribers, Topic Predicts Featuring, ANP Bottom-Performer Analysis. Added (5 new, 2026-04-02): MSN Formula Divergence, Formula × Topic Interaction, SmartNews Cross-Platform Formula Trap, Notification Outcome Language, Notification Send-Time. New 2026-04-08c: nature/wildlife dual-headline formula guidance (SN overperforms, AN underperforms — opposite routing); 0% featuring investigation (content type is constraint, no editorial lever). Author Playbooks page live (docs/author-playbooks/index.html). Headline Grader (docs/grader/index.html): daily via GitHub Actions (.github/workflows/grader.yml); Run Now button (passcode 8812); 15 criteria (added no_questions 2026-04-09; WTK hardened from LLM to objective rule p=3.0e-6); 30-day rolling history. Weekly auto-ingest pipeline built 2026-04-09: download_tarrow.py + weekly_ingest.yml (Monday 8pm CDT) + longitudinal snapshot store (data/weekly_snapshots.json). 19/19 smoke tests pass. Tarrow shared 2026 sheet with service account 2026-04-09 — pipeline fully operational. O&O/Amplitude layer is a separate sub-workstream.',
    blockers: [
  ...
  'p14-semrush': {
    ...
    systems: [
      { name: 'data-keywords repo', status: 'done', note: 'github.com/piercewilliams/data-keywords — implementation repo; full context seeded 2026-04-09' },
      { name: 'SEMrush API', status: 'pending', note: 'Access confirmed; API key held by Sarah Price. Full API docs in hand. No automation until Rocky/Julio confirm per-endpoint credit rates.' },
      { name: 'DataForSEO API', status: 'pending', note: 'Flagged by Rocky as cheaper alternative for batch use. Evaluate before committing SEMrush credits.' },
      { name: 'Sigma dashboard (Stephanie Z.)', status: 'pending', note: 'May already have keyword-to-article mapping — confirm before building from scratch.' },
    ],
    contacts: [
  ...
  'p7-tracker': {
    ...
    blockers: [
      'HOLD pending three remaining prerequisites: (1) Cluster ID field in CUE + WP (P6 dev / EGS-127), (2) Send-to-CUE functional (rq-send-to-cue — in progress, date TBD per Kat Sheplavy), (3) PGS-80 analytics live (draft-to-publish ratio + export event tracking). NOTE: Send-to-WP (rq-send-to-wp) is NOW LIVE as of 2026-04-09 for UsW, WW, SOD, Bargain Hunter — ✅ prerequisite satisfied.',
