...
export const PROJECTS = {
  ...
  'p3-headlines': {
    ...
    blockers: [
      ...
      'O&O/Amplitude layer: blocked on CSA eng p-tagging bug (separate workstream, not blocking core)',
    ],
    nextActions: [
  ...
  'p12-unitedrobots': {
    ...
    nextActions: [
      'Monitor Q/Cue integration code review (Daury) — when merged, rq-send-to-cue satisfied',
      'Monitor WordPress headline bug fix (Daury)',
      'Monitor prerequisites — no Pierce build action until all land',
  ...
  'p14-semrush': {
    ...
    compassGoal: 'G5 — Professional Development: Technical & Editorial Fluency',
    description: 'Two-layer keyword intelligence system. Layer 1 — Rocky\'s SEMrush gap analysis: domain-level competitor pulls (usmagazine.com vs. people.com etc.) identifying keywords competitors rank for that we don\'t. Us Weekly done (3.20.2026); financial services next per Chris. Layer 2 — data-keywords prototype (Pierce, piercewilliams/data-keywords): editorial decision layer on top — scores keyword candidates by volume × CPC impact, difficulty, content fit → Go Hard / Test Small / Skip verdicts. 14 briefs across 4 verticals ready to present Chris Palo Tue 4/14 12:30pm CDT. Integration: Rocky gap pulls feed keyword candidates into data-keywords briefs → verdicts feed cluster decisions. Chris framing: "layer this in related to domain/site decision making on a topic or story level as we decide on clusters."',
    blockers: [
    ...
    nextActions: [
      'Present prototype to Chris at 1:1 Tue 4/14 12:30pm CDT — URL: https://piercewilliams.github.io/data-keywords/',
      'Await Rocky gap pulls: 4 queued (Financial Health first — highest CPC), 1 per category, all 8 National Team sites combined. Category competitors confirmed 2026-04-13.',
      'Await Sara Vallone hot take on prototype (Slacked 2026-04-13)',
      'Rocky+Chad meeting Wed 4/15 1:30pm CDT — agenda: SEMrush credits, Sigma/Snowflake scope, keyword-to-article data',
  ...
  'p18-agentic-writing': {
    ...
    compassGoal: 'G2 — Editorial Standards & Voice Guidelines',
    description: 'Agent-in-document workflows for McClatchy editorial teams. Sparked by Chris Palo forwarding Every.to\'s "Proof" podcast (2026-04-13). CRITICAL FRAMING from Chris: "I don\'t care about Every.to\'s service specifically — I like the SYSTEM of having agents interact in documents." Proof (proofeditor.ai, open source) is a reference implementation, not the target product. The two workflows are fundamentally different design patterns: (1) PRD workflow — agent is a collaborator on planning docs: human + agent co-draft a plan, colleague\'s agent reviews + leaves comments, human approves, agent executes. Plans are short-lived artifacts; shareability > storage. (2) Content creation workflow — agent handles the labor layer while human owns the voice: record thoughts (Monologue app or text), agent builds a running note doc, agent creates outline, human writes into the outline or monologues verbatim and agent incorporates near-verbatim. Key editorial trust mechanism: provenance tracking (green gutter = human-written, purple = AI-written) lets writers see exactly what they contributed vs. what the agent contributed. Chris uses Monologue (audio-to-text app) personally. AX (Agent Experience) design principle: the tool must be optimized for agents as first-class users — low friction for agents to access and operate in docs (Proof: just paste a link, no login required). Agent trust prerequisite: "we need to get some agents that we feel good about" — this is a GATE, not a preference; we cannot deploy until we\'ve established which agents are trustworthy for McClatchy use cases. Primary target: Sara\'s team (content creation). Secondary: Susannah + Kat. GitHub repo TBD (Pierce creating 2026-04-14).',
    blockers: [
      'GitHub repo not yet created — Pierce creating 2026-04-14',
      'Agent trust gate: "agents we feel good about" not yet defined — must establish which agents are reliable before putting teams in this workflow',
      'Scope not defined — awaiting 1:1 with Chris to align on which workflow first and which team',
    ],
    nextActions: [
      ...
      'Identify and vet candidate agents — Proof (proofeditor.ai) as reference architecture; Chris uses Monologue for audio input; evaluate trustworthiness against McClatchy use cases',
      'Map PRD workflow loop for McClatchy: plan → agent review → human approve → agent execute (who are the agents? what are the docs?)',
      'Map content creation workflow: Monologue/text thoughts → agent running note doc → agent outline → human writes in',
    ],
