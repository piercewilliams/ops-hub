...
export const COMPLETED_TASKS = [
  { date: '2026-04-09', task: 'data-headlines weekly auto-ingest pipeline built: (1) download_tarrow.py — downloads live 2026 Google Sheet via Drive API using existing service account; 2025 static, never re-downloaded; (2) .github/workflows/weekly_ingest.yml — Monday 8pm CDT cron; compares xlsx to HEAD before triggering; commits xlsx + docs/ + data/ on change; (3) generate_site.py updated to write data/build_summary.json (8 tracked metrics: fb_wtk_sn_rank, fb_q_sn_rank, fb_sn_n, notif_len_best_ctr, sn_top_share, sn_channel_rows, an_median_hl_len, sn_median_hl_len) at end of every run; (4) update_snapshots.py — appends build_summary to data/weekly_snapshots.json for longitudinal tracking. Integrity pass: fixed xlsx-not-committed bug (change detection would re-fire every week), removed unused google-auth-httplib2, added requests to requirements.txt, corrected stale grader warning text (WTK now rule-based not LLM). README + REFERENCE.md updated. Smoke tests expanded: 19/19 pass. Pending: Tarrow must share sheet with pierce-tools-service-account@pierce-tools.iam.gserviceaccount.com (message sent).', project: '#3 T1 Headlines Analysis (Price)' },
